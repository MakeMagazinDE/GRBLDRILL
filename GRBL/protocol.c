...
void protocol_switch_report()
{
// Evaluate limit, aux & stop switch settings
  uint8_t switchflags = 0;  // output to status msg
  ...
  if (!(LIMIT_PIN & (1<<Z_LIMIT_BIT)))
    {
    switchflags |= 4;
    }
#endif

  if (!(AUX_PIN & (1<<AUX_16_BIT)))
    {
    switchflags |= 8;
    }
  if (!(AUX_PIN & (1<<AUX_17_BIT)))
    {
    switchflags |= 16;
    }
