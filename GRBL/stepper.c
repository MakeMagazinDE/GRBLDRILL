...
static volatile uint32_t servo_pulsecount;        // servo ISR counter
...
// already ran into limit switches to prevent damage -cm
  ...
  if (!(LIMIT_PIN & (1<<X_LIMIT_BIT)) && (out_bits & (1<<X_DIRECTION_BIT)))
    {
  ...
  if (!(LIMIT_PIN & (1<<Y_LIMIT_BIT)) && (out_bits & (1<<Y_DIRECTION_BIT)))
    {
  ...
  if (!(LIMIT_PIN & (1<<Z_LIMIT_BIT)) && (out_bits & (1<<Z_DIRECTION_BIT)))
    {
  ...
  // Zeitschleife zu Fuss, damit Timer2 frei wird   -cm
  static volatile uint8_t us;
  us = (settings.pulse_microseconds >> 1) + (settings.pulse_microseconds >> 2);
  while ( us-- ) {}

  // Reset stepping pins (leave the direction pins) - now here -cm
...
// Changed to RC servo control, pulse len now by delay in Timer1 ISR
// pulse nominally ranges from 1.0 ms to 2.0 ms with 1.5 ms center
// by -cm

ISR(TIMER2_OVF_vect)
{
  TCNT2 = 220; // Reload timer counter, ca. 20 µs
  TCCR2B = (1<<CS21); // Begin timer2. Full speed, 1/8 prescaler
  ...
  // Servo-Anfang
  if (sys.position[Z_AXIS] >= 0)  // positiv?
  	{
  	my_len = (sys.position[Z_AXIS] >> 3);
	}
  // Servo-Endanschlag
  if (my_len > 50)
  	{
  ...
  my_len = my_len + 49L; // ajust for 1.0 ms min. length

  if (servo_pulsecount == 1000L - my_len)
  	{
  	AUX_PORT |= (1 << AUX_SERVO_BIT); // set servo bit
  	}

  if (servo_pulsecount == 1000L)
  	{
    AUX_PORT &= ~(1 << AUX_SERVO_BIT); // clear servo bit
    servo_pulsecount = 0;
  	}
...
{
  AUX_PORT |= (1<<AUX_COUNTER_RESET);  // counter reset pin (should turn Vcc OFF)
  delay_ms(50);
  AUX_PORT &= ~(1<<AUX_COUNTER_RESET);
  }
...
void st_init()
{
  ...
  AUX_DDR = AUX_DDRVAL; // Auxiliary outputs
  AUX_PORT = ~AUX_DDRVAL;

  STEPPING_DDR |= STEPPING_MASK;
  STEPPING_PORT = (STEPPING_PORT & ~STEPPING_MASK) | settings.invert_mask;
  STEPPERS_ENABLE_DDR |= (1<<STEPPERS_ENABLE_BIT) | (1<<STEPPERS_ACTIVITY_BIT);  // -cm
  ...
  TIMSK2 |= (1<<TOIE2);
  // Timer2 jetzt für Servo benutzt
  TCNT2 = 220; // Reload timer counter, ca. 20 µs
  TCCR2B = (1<<CS21); // Begin timer2. Full speed, 1/8 prescaler
  st_counter_null();
